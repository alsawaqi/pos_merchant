...
it('creates an alternate unit with a factor', function (): void {
    ...
    expect($res->json('data.name'))->toBe('kg');
    expect($res->json('data.factor'))->toBe('1000.0000');
    expect(IngredientAltUnit::where('ingredient_id', $ing->id)->where('name', 'kg')->exists())->toBeTrue();
});

it('lists an ingredient units and surfaces them on the ingredient resource', function (): void {
    $ctx = makeMerchantActor();
    $ing = makeIngredient($ctx['company']);
    $this->postJson("/api/ingredients/{$ing->uuid}/units", ['name' => 'kg', 'factor' => '1000'])->assertCreated();
...
it('refuses a duplicate active unit name', function (): void {
    ...
    $this->postJson("/api/ingredients/{$ing->uuid}/units", ['name' => 'kg', 'factor' => '999'])->assertStatus(422);
});

it('restores a soft-deleted unit when its name is re-created', function (): void {
    $ctx = makeMerchantActor();
    $ing = makeIngredient($ctx['company']);
    $created = $this->postJson("/api/ingredients/{$ing->uuid}/units", ['name' => 'kg', 'factor' => '1000'])->json('data');
    ...
    $again = $this->postJson("/api/ingredients/{$ing->uuid}/units", ['name' => 'kg', 'factor' => '1200'])->assertCreated();
    expect($again->json('data.factor'))->toBe('1200.0000');
    expect(IngredientAltUnit::where('ingredient_id', $ing->id)->where('name', 'kg')->count())->toBe(1);
});
