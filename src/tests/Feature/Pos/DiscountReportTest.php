...
it('returns empty by_rule + by_staff arrays with Phase 8 note', function (): void {
    makeMerchantActor();

    $response = $this->getJson('/api/reports/discounts?date_from=2026-06-01&date_to=2026-06-30')->assertOk();

    expect($response->json('data.by_rule'))->toBe([]);
    expect($response->json('data.by_staff'))->toBe([]);
    expect($response->json('data._phase.by_rule_stub'))->toContain('Phase 8');
});
